...
export function LoginForm({ onSubmit, errorMsg }: { onSubmit: (data: LoginFormValues) => Promise<void>; errorMsg: string }) {
  ...
  return <form noValidate onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-6">
    ...
    <Button type="submit" size="lg" className="w-full" disabled={isSubmitting}>{isSubmitting ? <><LoaderCircle className="animate-spin" data-icon="inline-start" />Menghubungkan...</> : <>Masuk ke workspace<ArrowRight data-icon="inline-end" /></>}</Button>
  </form>;
}
